...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['content'])) {
    ...
    if (!empty($_POST['edit_id'])) {
        $stmt = $pdo->prepare("UPDATE contact_contents SET content = :content WHERE id = :id");
        $stmt->execute([
            ':content' => $content,
            ':id' => $_POST['edit_id']
        ]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO contact_contents (content) VALUES (:content)");
...
if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM contact_contents WHERE id = :id");
...
if (isset($_GET['edit'])) {
    $editId = $_GET['edit'];
    $stmt = $pdo->prepare("SELECT * FROM contact_contents WHERE id = :id");
    $stmt->execute([':id' => $editId]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $editContent = $row ? $row['content'] : '';
}
?>
          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
              <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6 class="dark:text-white">Contact Contents</h6>
              </div>
              <?php 
              ...
              <form action="" method="POST" class="mt-2 mb-10">
                    <textarea name="content" id="editor"><?php echo htmlspecialchars($editContent); ?></textarea>
                    <?php if ($editId): ?>
                    ...
                    <!-- <div class="flex-none max-w-full px-3 text-right"> -->
                    <!-- <button class="inline-block px-8 py-2 mb-0 text-xs font-bold leading-normal text-center text-blue-500 align-middle transition-all ease-in bg-transparent border border-blue-500 border-solid rounded-lg shadow-none cursor-pointer bg-150 active:opacity-85 hover:-translate-y-px tracking-tight-rem bg-x-25 hover:opacity-75"><a href="addProduct.php">Add Product</a></button> -->
                    <button class="inline-block px-8 py-2 mb-0 text-xs font-bold leading-normal text-center text-blue-500 align-middle transition-all ease-in bg-transparent border border-blue-500 border-solid rounded-lg shadow-none cursor-pointer bg-150 active:opacity-85 hover:-translate-y-px tracking-tight-rem bg-x-25 hover:opacity-75" type="submit"><?php echo $editId ? 'Update' : 'Publish'; ?></button>
                  <!-- </div> -->
              </form>
                ...
                <div class="p-0 overflow-x-auto">
                  <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                    <thead class="align-bottom">
                      <tr>
                          <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-sm border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Content (Shortent)</th>
                          <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-sm border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Created At</th>
                    ...
                    <tbody>
                    <?php
                    $stmt = $pdo->query("SELECT * FROM contact_contents ORDER BY id DESC");
                    ...
                    ?>
                      <tr>
                        ...
                        >
                          <span class="bg-gradient-to-tl  px-2.5 text-xs dark:text-white rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline leading-none"><?php echo substr(strip_tags($row['content']), 0, 50) . '...'; ?></span>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400"><?php echo $row['created_at']; ?></span>
                        </td>
                        <td class="px-6 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <a href="javascript:;" class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400"> <?php echo $row['id']; ?> </a>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <a name="deleteBtn" class="relative z-10 inline-block px-2 py-2.5 mb-0 font-bold text-center text-transparent align-middle transition-all border-0 rounded-lg shadow-none cursor-pointer leading-normal text-sm ease-in bg-150 bg-gradient-to-tl from-red-600 to-orange-600 hover:-translate-y-px active:opacity-85 bg-x-25 bg-clip-text" href="?delete=<?php echo $row['id']; ?>" onclick="return confirm('Delete this entry?');"><i class="mr-2 far fa-trash-alt bg-150 bg-gradient-to-tl from-red-600 to-orange-600 bg-x-25 bg-clip-text"></i>Delete</a>
                        <a class="inline-block dark:text-white px-2 py-2.5 mb-0 font-bold text-center align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-normal text-sm ease-in bg-150 hover:-translate-y-px active:opacity-85 bg-x-25 text-slate-700" href="?edit=<?php echo $row['id']; ?>"><i class="mr-2 fas fa-pencil-alt text-slate-700" aria-hidden="true"></i>Edit</a>
                        </td>
                      </tr>
                      <?php endwhile; ?>
                    </tbody>
